...
:::info[确认这条安全提示的措辞]
这条安全提示是新撰写的文案，并非引用自现有文档。它基于内部产品的设计考量：如果一枚铭文与 alkanes 共享同一个 UTXO，就存在被销毁的风险，而且 SUBFROST 并不索引 `ord`。请确认团队希望使用的确切措辞，以及是否要引用正在开发中的应用内"将 ordinals 拆分到新钱包"提示。
:::
...
:::info[解包确认延迟（"3 个区块"）]
当兑换回原生 BTC 时，现有文档中提到有一个安全确认期（"在 SUBFROST 协议发送解包后的 BTC 之前，有一个 3 个区块的确认期"）。这个数字没有得到任何内部资料的印证。在正式引用之前，请先确认真实数值。
:::
