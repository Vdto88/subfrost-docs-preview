...
- **DIESEL：发行量与真实 Bitcoin 挂钩。** DIESEL 是协议的原生发行代币。它的铸造与 Bitcoin 矿工费同步（最多为区块奖励的一半），并遵循与新增比特币相同的减半时间表。换句话说，DIESEL 的供应量锚定于真实的 Bitcoin 出块情况，而不是随意增发的。

- **FIRE：奖励与治理。** FIRE 奖励为 DIESEL/frBTC 资金池提供流动性的人，并支持质押（staking）与债券（bonds）。这是活跃参与者赚取协议份额、并随时间参与治理的方式。

:::info[DIESEL、frBTC 与 FIRE 的关系（"Theory"）]
项目负责人要求撰写一个专门的"Theory"章节，解释这三者在经济上如何相互配合（发行量与矿工费挂钩、frBTC 与 DIESEL 之间的储备关系，以及 FIRE 为何存在）。这套完整的经济学论述目前尚未在任何单一资料中写明，因此本"从这里开始"页面故意将其省略，并标记为需要在 **代币与经济学** 下建立的页面。请勿在此处臆造储备比率的机制细节。
:::
...
:::info[发布前请对照实际运行中的应用核实已上线/规划中的划分]
现有文档中有几处以现在时描述规划中的功能（跨链兑换、vault）。请通过打开正式应用并核对链上数据来核实每一项的状态，并将尚未上线的内容标注为**规划中**。来源：`Diagnostico-docs.md` 第 10 节，代币相关的资料汇编。
:::
