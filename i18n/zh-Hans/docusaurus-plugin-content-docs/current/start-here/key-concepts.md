...
:::info[兑换手续费数字]
在确认之前，请勿公布具体的 AMM 兑换手续费。一个通用的参考教程展示了 0.3% 作为示例费率，但那只是一个通用教程，并未确认为 SUBFROST 正式资金池的费率。另外，frBTC 的包装/解包收取 0.1% 的费用，不过这个数字在使用前也应重新确认是否仍然准确。内部流传的"1%（0.8% LP + 0.2% 销毁）"这个数字未在任何资料中找到依据。
:::
