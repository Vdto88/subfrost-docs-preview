...
:::info[发布前请确认精确的发行数字]
来源资料对该规则仅作了定性描述（与矿工手续费等量、最高为区块奖励的 50%、与新聪的发行计划相同），并提到大约一年运行时间里发行了约 900 枚 DIESEL。目前没有明确写明的、以区块数或年数计的 DIESEL 减半周期；它隐含地遵循 Bitcoin 的计划。在给出任何具体数字之前，请对照创世合约（genesis contract）确认精确数据。
:::
