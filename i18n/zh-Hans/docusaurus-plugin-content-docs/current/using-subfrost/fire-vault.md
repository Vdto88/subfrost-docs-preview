...
退出 cFIRE 金库会销毁你的 **cFIRE-POS** NFT，换回属于你的 FIRE（本金 + 回流），扣除 **10%**：
...
| 风险 | 对你意味着什么 |
| --- | --- |
| **目前仍是受信任的预言机** | 债券所依据的参考值由一个 2/3 签名者集合推送上链。让它在更多签名者之间联邦化是设计意图，但这一步尚未完成 |
| **锁仓流动性受限** | 锁仓的 LP 在到期前无法取出。如果需要部分灵活性，可以拆分为 FIRE-PT |
| **区块时间存在波动** | 这里的所有时长都假设 Bitcoin 平均约 10 分钟一个区块，实际经过的时间会有偏差 |
| **cFIRE 金库本身不产生收益** | 它持有的是普通的 FIRE 代币，不做任何耕作。唯一能推高汇率的，是别人离开并支付抽成 |
...
- **要发送的是投票代币（cFIRE-DELEGATE），绝不是头寸（cFIRE-POS）。** 头寸就是那笔存款。
