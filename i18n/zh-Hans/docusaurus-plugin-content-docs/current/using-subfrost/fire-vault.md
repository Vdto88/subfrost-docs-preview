...
关于收益从何而来，这里必须说得直白，因为它与本页其他任何内容都不同：**cFIRE 金库本身不产生任何收益。** 它持有的是一个普通代币，没有任何东西拿它去耕作。在两次退出之间，它完全是零和的。它支付的，是离开的人留下的东西。

### 池子的运作方式

存入 FIRE，你会按当前汇率获得 **cFIRE**。退出时，你会留下 **8% 的抽成**进入一个池子。这个池子不归协议所有：它会以大约一个 Bitcoin 日为间隔，在大约一年的时间里逐步回流到汇率中，惠及所有仍留在金库里的人。

| 参数 | 数值 |
| --- | --- |
| 基础资产 | FIRE |
| 退出抽成（普通存入） | 8%（800 bps） |
| 回流间隔 | 144 个区块（约一个 Bitcoin 日） |
| 释放期数 | 365 |
| 小数位数 | 8，继承自 FIRE |
...
:::warning[两种代币都是持票人资产]
谁持有**头寸**，谁就能退出它，所以发送头寸等于发送这笔存款。发送**投票代币**只发送投票权。两者不是一回事，而且只有其中一个是用来传递的。
:::
