...
债券认购适合那些想要**立即**获得 FIRE，而不是随时间慢慢赚取的人。你将 LP 代币永久交给协议，作为交换，你会以**相对于市场价格 10% 的折扣**获得 FIRE，并在约 **14 天**（2,100 个区块）内归属（vest）。该折扣是协议可以调整的一项设置，因此请将 10% 视为当前费率，而不是固定规则。

用于认购债券的 LP 会直接进入国库并留在那里。它不会被退还，并且会永久性地为赎回地板价提供支撑。

用大白话说，债券认购就是这样一笔交易：你把 LP 永久交给协议，换来的是以相对于市场价格 10% 的折扣获得的 FIRE，并在 14 天内归属。
...
一个认购债券后立即赎回的人，净收益为零。14 天的归属期意味着债券认购者是在押注价格上涨，而不是从协议中抽取价值。
...
以上内容讲的都是通过质押 **LP** 来赚取 FIRE。**cFIRE 金库**是另一个金库，它接收的是 **FIRE** 本身。

收益从哪里来？来自比你更早退出金库的头寸。与本页其他任何内容都不同，cFIRE 本身不产生内在收益：收益来自活动本身。

### 池子的运作方式

存入 FIRE，你会获得一个持有你份额的 **cFIRE-POS** NFT 头寸。退出时，你会留下 **8% 的抽成**进入一个池子。这个池子不归协议所有：它会每 144 个区块（一个 Bitcoin 日）一次，在大约一年的时间里，逐步回流给尚未退出的现有 cFIRE-POS 持有者。这就是收益的来源。

因此汇率只朝一个方向移动，而且是因为有人离开才移动。比身边的人待得更久，就是这里策略的全部。

### cFIRE 要解决的问题，以及解决它的头寸

投票权是**按地址**读取的。放在金库里的 FIRE 不在你的地址上，所以普通存入之后，你在治理中读数为**零**。你是在收益和投票权之间二选一。

通过**头寸（position）**存入，就是同时拿到两者的方式。

| | 普通存入 | 通过头寸存入 |
| --- | --- | --- |
| 你持有什么 | 你地址上的 cFIRE | 一个 `cFIRE-POS` NFT，外加一个投票代币 |
| 收益 | 金库汇率 | 完全相同的金库汇率 |
| 是否计入治理 | 否 | 是，通过投票代币 |
| 退出成本 | 8% | 10% |

头寸赚取的收益和普通存入完全一样。那两个百分点，就是投票权的价格。
...
```
Deposit(FIRE) -> cFIRE-POS-{n}            头寸：持有金库份额
              -> cFIRE-DELEGATE-{n}-{m}   投票权：可转让，承载权重
```

**头寸**持有份额。它赚取金库所赚的一切，而它本身不投任何票。**投票代币**承载该头寸的权重，是一个普通的可转让代币。

有一个合约在一次调用中完成这两件事，它会从铸造结果中读出头寸，因此投票权不会被落下。

### 你的权重

你的权重是你的头寸**以 FIRE 计**的价值，所以等值的头寸和普通 FIRE 余额，权重相同。

它是在扣除退出费用**之前**计量的：一个退出后能拿到 90 FIRE 的头寸，权重是 100。而且随着池子回流到汇率中，它会自行上升，你无需做任何操作。

### 委托就是一次转账

委托背后没有注册表，也没有签名方案。投票代币是**持票人（bearer）**资产，谁持有它，谁就用它投票。所以委托就是一次普通的转账。

| 操作 | 链上发生了什么 |
| --- | --- |
| **自己保留** | 你持有代币，由你投票 |
| **发送出去** | 收到它的人用你的权重投票 |
| **委托给自己** | 头寸在同一笔交易中向你铸造一枚**新的**投票代币，并作废旧的那一枚 |
| **取消** | 头寸清除其委托。旧代币原封不动地留在持有者的钱包里，只是不再计票 |

铸造替代代币会作废先前的那一枚，**无论它在哪里**。持有它的人并没有被拿走任何东西：代币仍在他们的钱包里，只是从那一刻起读数为零。

:::warning[两种代币都是持票人资产]
谁持有头寸（**cFIRE-POS**），谁就能退出它，所以发送头寸等于发送这笔存款。发送投票代币只发送投票权。**除非你真的想把自己的 FIRE 送走，否则绝不要发送 cFIRE-POS NFT。** 唯一用来传递的 NFT 是 **cFIRE-DELEGATE**。
:::

### 退出 cFIRE 金库

销毁头寸，取回你的 FIRE，扣除 **10%**：
...
头寸会被销毁，不会作为凭据保留，任何为它投票的代币也将不再计票。

这个 10% 是合约层面的不变量，而不是一个展示用的数字。除非金库自身的抽成加上包装合约的两条 1% 分支恰好等于 10%，否则工厂合约会拒绝绑定该金库，因此这个拆分不可能与本页所述产生偏差。
...
:::info[治理尚未读取这些代币]
现在铸造投票代币是为将来做好准备。当计票逻辑改为读取它们时，它才开始计入。本页其余内容，包括收益和退出，今天都已生效。
:::
...
| **cFIRE 金库本身不产生收益** | 它持有一个普通代币，不做任何耕作。唯一能推高汇率的，是别人离开并支付抽成 |
| **投票代币是持票人资产** | 发错地址，投票权就没了。铸造替代代币是唯一的补救方式，而且要花一笔交易 |
| **cFIRE 的投票尚未计入** | 治理目前不读取这些代币。现在铸造是为它开始读取时做好准备 |
...
- **不要混淆这两个金库。** 上面的 FIRE 金库接收 DIESEL / frBTC LP；cFIRE 金库接收 FIRE 本身，它的收益完全来自其他人的退出抽成。
- **如果你打算投票，就通过头寸存入。** 普通存入收益相同，但在治理中读数为零，而这个差别在退出时要多付两个百分点。
- **要发送的是投票代币，绝不是头寸。** 头寸就是那笔存款。
