...
当前的索引器运行时（v10）通过解耦三个各自独立扩展的层来实现水平扩展：

- **状态（TiKV）。** 状态存储在 TiKV 中，这是一个通过网络查询的分布式键值存储，而不是打包进每个实例的本地数据库。多个实例共享同一个后端，运行时可以在一次网络往返中批量获取多个键。
- **索引器（subshrew）。** 索引进程独立运行，与状态层解耦。同一套运行时也可以作为单一二进制文件，以 **rockshrew-mono** 的形式运行。
- **View / RPC 层。** 运行 view 函数的进程是独立的，可以扩展到多个副本，每个副本各自加载 WASM 并针对共享状态进行求值。

实际意义在于：读取吞吐量可以扩展，而无需把全部状态复制到每一台机器上。

## 运行索引器

你可以在两种形态之间选择：

- **`rockshrew-mono`**，一个将运行时与本地数据库打包在一起的单一二进制文件。这是运行单个节点最简单的方式。
- **`subshrew`**，面向 TiKV 的 v10 索引器，适用于你需要解耦、可水平扩展的部署方式时。

从创世区块开始同步大约需要数天时间。要跳过这一过程，可以从一个**已发布的快照**引导启动，然后让节点追上链尖，这样只需数小时而非数天。

:::info[发布前请确认运行命令与快照位置]
请提供 `rockshrew-mono` 和 `subshrew` 的确切启动命令（包括 Bitcoin RPC 数据源、数据或 TiKV 端点、以及 WASM 路径的相关标志位），以及公开快照的 URL。这些信息来自节点二进制文件和 flex 团队，尚未在现有资料中被完整记录。如果 `subfrost-node` 是运行签名者或全节点的打包方式，也请在此处附上它的 `--help` 输出。来源：关于 metashrew v10（subshrew/TiKV/rockshrew-mono、快照）的 vault 笔记，待 flex 团队确认。
:::

## 构建你自己的索引器 WASM

metashrew 可以运行任何实现了其宿主接口（host interface）的 WASM 程序，因此你可以编写自定义的索引逻辑，而不局限于运行 Alkanes 程序本身。一个程序处理每个区块，并通过运行时写入键值状态，同时暴露供读取使用的 view 函数。在 v10 中，运行时是异步的，会为长时间运行的请求设置超时，并且可以在不破坏接口的前提下新增宿主函数。

v10 的并行化带来了一个需要注意的地方：执行可能被分片，因此旧有的“同一份 WASM，同一份索引结果”保证不再自动成立。如果你编写的索引器依赖执行顺序，请以**可串行化语义（serializable semantics）**来编写它，使结果不依赖于执行顺序。
