...
如今的托管由**九名签名者**持有，且网络处于许可制阶段：由这个固定群体中达到法定人数（quorum）的签名者共同授权每一次解包，这些签名者目前尚未公开身份。

从现状到目标，有三个步骤：

1. **签名者可被识别。** 密钥会按需迁移，使每一名签名者都能被公开识别身份。
2. **更大的集合。** 签名者集合会扩展到超过当前的九人规模。
3. **无许可质押。** 参与门槛开放，签名者集合不再是一个固定群体。
