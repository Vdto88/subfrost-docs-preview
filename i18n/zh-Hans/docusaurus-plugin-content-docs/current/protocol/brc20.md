...
:::info[发布前确认具体的铭文与交易流程]
旧文档描述的是一个三笔交易的部署流程（commit、reveal、activation），以及具体的 reveal 脚本细节，还有硬编码的合约地址和一个 `wrapAndExecute2` 入口点，其签名在两个页面中并不一致。ord 的规范铭文流程是先 commit 后 reveal。发布前请对照 brc20-prog / frBTC 合约源码，确认真实的交易模式、入口点和任何地址，并把开发层面的具体细节留在开发者指南中，以便对照仓库进行验证。来源：`Diagnostico-docs.md` §10 C。
:::
