...
This section keeps things simple on purpose. Terms like protostone, cellpack, and the exact contract model are covered in the [Protocol](../protocol/how-subfrost-works) section, and the developer tooling (CLI, SDK, JSON-RPC) is in [Build on SUBFROST](../build/overview) and the [API & SDK Reference](../api-reference/getting-started/overview).
:::
