...
While all communication on the `subp2p` network is encrypted at the transport layer (using the `noise` protocol for the handshake), `gossipsub` itself does not provide end-to-end encryption for messages. This means that any peer that is subscribed to a topic can read all the messages that are published to that topic.
